...
function getCfg() {
  ...
  // Provider mode: when the install ships with a central Meta app in .env
  // (META_APP_ID / META_APP_SECRET / FB_CONFIG_ID), those are the defaults —
  // the client never has to create an app or touch credentials.
  if (!(c.whatsapp_app_id || '').trim() && process.env.META_APP_ID) c.whatsapp_app_id = process.env.META_APP_ID;
  if (!(c.whatsapp_app_secret || '').trim() && process.env.META_APP_SECRET) c.whatsapp_app_secret = process.env.META_APP_SECRET;
  if (!(c.fb_config_id || '').trim() && process.env.FB_CONFIG_ID) c.fb_config_id = process.env.FB_CONFIG_ID;
  return c;
}
function isProviderApp() {
...
module.exports = {
  GRAPH, SCOPES, getCfg, isConfigured, isProviderApp, loginUrl, waLoginUrl, exchangeCode, longLived,
